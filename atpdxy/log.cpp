...
#define FORMAT_ITEM_MAPPING(str, C) \
        {#str, [](const std::string& fmt) { return FormatItem::ptr(new C(fmt));}}

        FORMAT_ITEM_MAPPING(m, MessageFormatItem),
        FORMAT_ITEM_MAPPING(p, LevelFormatItem),
        FORMAT_ITEM_MAPPING(r, ElapseFormatItem),
        FORMAT_ITEM_MAPPING(c, NameFormatItem),
        FORMAT_ITEM_MAPPING(t, ThreadIdFormatItem),
        FORMAT_ITEM_MAPPING(n, NewLineFormatItem),
        FORMAT_ITEM_MAPPING(d, DateTimeFormatItem),
        FORMAT_ITEM_MAPPING(f, FileNameFormatItem),
        FORMAT_ITEM_MAPPING(l, LineFormatItem),
        FORMAT_ITEM_MAPPING(T, TabFormatItem),
        FORMAT_ITEM_MAPPING(F, FiberIdFormatItem),
        FORMAT_ITEM_MAPPING(N, ThreadNameFormatItem),
#undef FORMAT_ITEM_MAPPING
    };
...
struct LogIniter {
    LogIniter() {
        g_log_defines->addListener([](const std::set<LogDefine>& old_value,
            ...
            INFO(GET_ROOT_LOGGER()) << "on_logger_conf_changed";
            for(auto& i : new_value) {
                auto it = old_value.find(i);
                atpdxy::Logger::ptr logger;
                if(it == old_value.end()) {
                    logger = GET_LOGGER_BY_NAME(i.name);
                } else {
                    if(!(i == *it)) {
                        logger = GET_LOGGER_BY_NAME(i.name);
                    } else {
            ...
            for(auto& i : old_value) {
                ...
                if(it == new_value.end()) {
                    auto logger = GET_LOGGER_BY_NAME(i.name);
