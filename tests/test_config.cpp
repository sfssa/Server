...
void print_yaml(const YAML::Node& node, int level) {
    if(node.IsScalar()) {
        INFO(GET_ROOT_LOGGER()) << std::string(level * 4, ' ')
...
void test_yaml() {
    ...
    //print_yaml(root, 0);
    //SYLAR_LOG_INFO(GET_ROOT_LOGGER()) << root.Scalar();

    INFO(GET_ROOT_LOGGER()) << root["test"].IsDefined();
    INFO(GET_ROOT_LOGGER()) << root["logs"].IsDefined();
    INFO(GET_ROOT_LOGGER()) << root;
}

void test_config() {
    INFO(GET_ROOT_LOGGER()) << "before: " << g_int_value_config->getValue();
    INFO(GET_ROOT_LOGGER()) << "before: " << g_float_value_config->toString();

#define XX(g_var, name, prefix) \
    { \
        auto& v = g_var->getValue(); \
        for(auto& i : v) { \
            INFO(GET_ROOT_LOGGER()) << #prefix " " #name ": " << i; \
        } \
        INFO(GET_ROOT_LOGGER()) << #prefix " " #name " yaml: " << g_var->toString(); \
    }
...
#define XX_M(g_var, name, prefix) \
    { \
        auto& v = g_var->getValue(); \
        for(auto& i : v) { \
            INFO(GET_ROOT_LOGGER()) << #prefix " " #name ": {" \
                    << i.first << " - " << i.second << "}"; \
        } \
        INFO(GET_ROOT_LOGGER()) << #prefix " " #name " yaml: " << g_var->toString(); \
    }
    ...
    INFO(GET_ROOT_LOGGER()) << "after: " << g_int_value_config->getValue();
    INFO(GET_ROOT_LOGGER()) << "after: " << g_float_value_config->toString();
...
void test_class() {
    INFO(GET_ROOT_LOGGER()) << "before: " << g_person->getValue().toString() << " - " << g_person->toString();

#define XX_PM(g_var, prefix) \
    { \
        auto m = g_person_map->getValue(); \
        for(auto& i : m) { \
            INFO(GET_ROOT_LOGGER()) <<  prefix << ": " << i.first << " - " << i.second.toString(); \
        } \
        INFO(GET_ROOT_LOGGER()) <<  prefix << ": size=" << m.size(); \
    }

    g_person->addListener([](const Person& old_value, const Person& new_value){
        INFO(GET_ROOT_LOGGER()) << "old_value=" << old_value.toString()
                << " new_value=" << new_value.toString();
    });

    XX_PM(g_person_map, "class.map before");
    INFO(GET_ROOT_LOGGER()) << "before: " << g_person_vec_map->toString();

    YAML::Node root = YAML::LoadFile("/home/atpdxy/GitHub/Server/bin/conf/logs.yml");
    atpdxy::Config::LoadFromYaml(root);

    INFO(GET_ROOT_LOGGER()) << "after: " << g_person->getValue().toString() << " - " << g_person->toString();
    XX_PM(g_person_map, "class.map after");
    INFO(GET_ROOT_LOGGER()) << "after: " << g_person_vec_map->toString();
}

void test_log() {
    static atpdxy::Logger::ptr system_log = GET_LOGGER_BY_NAME("system");
...
// void test_loadconf() {
//     atpdxy::Config::LoadFromConfDir("conf");
// }
...
int main(int argc, char** argv) {
    ...
    // test_class();
    // test_log();
    // atpdxy::EnvMgr::GetInstance()->init(argc, argv);
    // test_loadconf();
    // std::cout << " ==== " << std::endl;
    // sleep(10);
    // test_loadconf();
    ...
    atpdxy::Config::Visit([](atpdxy::ConfigVarBase::ptr var) {
        INFO(GET_ROOT_LOGGER()) << "name=" << var->getName()
